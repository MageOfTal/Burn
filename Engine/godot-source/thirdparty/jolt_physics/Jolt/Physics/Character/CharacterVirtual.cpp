...
// TEMP DEBUG: tracing MoveShape's post-solve verification sweep zeroing moves.
...
void CharacterVirtual::DetermineConstraints(TempContactList &inContacts, float inDeltaTime, ConstraintList &outConstraints) const
{
	for (Contact &c : inContacts)
	{
		...
		// Steep-slope handling: mark the constraint for diagnostics, but do NOT add upstream
		// Jolt's secondary vertical-plane constraint (removed 2026-06-11, together with the
		// horizontal-velocity cancel in SolveConstraints). The secondary plane blocked ALL
		// horizontal motion into a too-steep face — including legitimate momentum at a
		// walkable→steep transition, which it converted into a vertical pop off the surface.
		// Steep faces are now plain tangent projections: momentum slides up them and gravity
		// drains it. Input-driven climbing is prevented script-side (wall-projection trig cap
		// + per-frame gravity + steep ground not counting as grounded for walk-accel).
...
bool CharacterVirtual::HandleContact(Vec3Arg inVelocity, Constraint &ioConstraint, float inDeltaTime)
{
	...
	// Impulse P = M dv
	float impulse_uncapped = delta_velocity / inv_effective_mass; // TEMP DEBUG: keep uncapped value for logging
	...
	float impulse = min(impulse_uncapped, max_impulse);

	// Calculate the world space impulse to apply. Applied along the contact normal at the contact
	// point — the physically correct rigid-body response. The downward component (when n points up-
	// and-toward-character, e.g., pushing a body below us) used to be cancelled here on the
	// rationale "we apply gravity later," but that ablation bends F off the contact normal, and
	// since the impulse is delivered at contact_pos (lever arm r = R·n for a sphere), the bent
	// force produces a torque (r × F ≠ 0) that spins the body — a spin which then feeds back into
	// the character's mass-ratio velocity clip via the body's contact-point velocity (linear + ω×r)
	// and launches the character upward. The downward component of the impulse on a body resting on
	// the floor is absorbed by the floor's normal force (Newton's 3rd law); the body slides
	// horizontally as expected. No other major character controller (Bullet, PhysX, Unity DOTS,
	// Unreal, Godot, Quake) performs this cancellation; it's a Jolt idiosyncrasy with no academic
	// citation.
	...
	// TEMP DEBUG: log impulse delivery for Dynamic-body contacts so we can audit cumulative push per
	// body per frame. Only log when the character is moving meaningfully (filter out idle gravity-stick
	// micro-impulses). Format keeps everything on one line per call so post-processing can group by
	// body ID and frame.
	{
		const float h_speed_sq = inVelocity.GetX() * inVelocity.GetX() + inVelocity.GetZ() * inVelocity.GetZ();
		if (h_speed_sq > 1.0f) {
			const float body_mass_for_log = (inverse_mass > 0.0f) ? (1.0f / inverse_mass) : 0.0f;
			const float m_eff_for_log = (inv_effective_mass > 0.0f) ? (1.0f / inv_effective_mass) : 0.0f;
			const Vec3 lever = Vec3(contact.mPosition - center_of_mass);
			::print_line(::vformat("[HC-PUSH] body=%d m=%.2f m_eff=%.2f dt=%.4f inV=(%.2f,%.2f,%.2f) contactV=(%.2f,%.2f,%.2f) n=(%.2f,%.2f,%.2f) proj=%.3f dist=%.4f delta_v=%.3f imp_unc=%.2f imp=%.2f cap=%.2f world_imp=(%.2f,%.2f,%.2f) r=(%.3f,%.3f,%.3f) |r|=%.3f",
					(int64_t)contact.mBodyB.GetIndexAndSequenceNumber(),
					body_mass_for_log, m_eff_for_log, inDeltaTime,
					(float)inVelocity.GetX(), (float)inVelocity.GetY(), (float)inVelocity.GetZ(),
					(float)contact.mLinearVelocity.GetX(), (float)contact.mLinearVelocity.GetY(), (float)contact.mLinearVelocity.GetZ(),
					(float)contact.mContactNormal.GetX(), (float)contact.mContactNormal.GetY(), (float)contact.mContactNormal.GetZ(),
					projected_velocity, contact.mDistance, delta_velocity,
					impulse_uncapped, impulse, max_impulse,
					(float)world_impulse.GetX(), (float)world_impulse.GetY(), (float)world_impulse.GetZ(),
					(float)lever.GetX(), (float)lever.GetY(), (float)lever.GetZ(), (float)lever.Length()));
		}
	}
	...
	)
{
	...
	// TEMP DEBUG: trace why SolveConstraints can return ~0 displacement with a real velocity.
	const bool dbg_on = (inVelocity.GetX() * inVelocity.GetX() + inVelocity.GetZ() * inVelocity.GetZ()) > 4.0f;
	::String dbg_trace;
	uint dbg_iter = 0;
	#define DBG_SC_EXIT(reason) do { if (dbg_on && outDisplacement.GetX() * outDisplacement.GetX() + outDisplacement.GetZ() * outDisplacement.GetZ() < 1.0e-6f) { \
		::print_line(::vformat("[SC] exit=%s iter=%d ncon=%d nprev=%d in_vel=(%.3f,%.3f,%.3f) vel=(%.3f,%.3f,%.3f) lastv=(%.5f,%.5f,%.5f) dot=%.6f disp=(%.5f,%.5f,%.5f) tRem=%.4f | %s", \
			reason, (int)dbg_iter, (int)ioConstraints.size(), num_previous_contacts, \
			(float)inVelocity.GetX(), (float)inVelocity.GetY(), (float)inVelocity.GetZ(), \
			(float)velocity.GetX(), (float)velocity.GetY(), (float)velocity.GetZ(), \
			(float)last_velocity.GetX(), (float)last_velocity.GetY(), (float)last_velocity.GetZ(), (float)velocity.Dot(last_velocity), \
			(float)outDisplacement.GetX(), (float)outDisplacement.GetY(), (float)outDisplacement.GetZ(), \
			inTimeRemaining, dbg_trace)); } } while (0)

	// Loop for a max amount of iterations
	for (uint iteration = 0; iteration < mMaxConstraintIterations; iteration++)
	{
		dbg_iter = iteration + 1; // TEMP DEBUG: count of iterations actually entered
		...
		// Compute how much of the into-contact velocity this constraint cancels.
		// For Static / Kinematic / infinite-mass contacts the full velocity-into-the-plane
		// is cancelled (clip_frac = 1) — they act like walls. For a finite-mass Dynamic
		// body, the character and body share the incoming momentum based on the body's
		// effective mass *at the contact point along the contact normal*:
		//     1/m_eff = 1/m_body + (r × n)·I⁻¹·(r × n)
		// where r = contact_pos − COM, n = contact normal, I = inertia tensor (world).
		// clip_frac = m_eff / (m_eff + m_char). This is the standard rigid-body constraint
		// formulation and Jolt computes the same quantity in HandleContact (line 745-746)
		// to size the push impulse. Using m_body here while HandleContact uses m_eff leaks
		// energy on off-center hits: my clip transfers velocity assuming the body's full
		// linear mass resists, but HandleContact only delivers an impulse sized to m_eff
		// (smaller for off-center hits, because part of the push spins the body up instead
		// of moving it). That energy shows up next frame as spin-amplified contact-point
		// velocity (v_linear + ω × r), and without the matching m_eff in the clip the
		// formula double-counts it — feedback loop that "launches" light spinning props.
		// With m_eff used consistently in both halves the math is self-consistent: a center
		// hit is identical to the old behavior (m_eff = m_body, r × n = 0); an off-center
		// hit yields more aggressively (smaller clip), and the resulting spin next frame
		// fits the energy budget the impulse delivered. No threshold needed.
		float clip_frac = 1.0f;
		if (constraint->mContact->mMotionTypeB == EMotionType::Dynamic)
		{
			BodyLockRead lock(mSystem->GetBodyLockInterface(), constraint->mContact->mBodyB);
			if (lock.SucceededAndIsInBroadPhase())
			{
				const Body &body = lock.GetBody();
				const MotionProperties *mp = body.GetMotionProperties();
				const float inv_body_mass = mp->GetInverseMass();
				if (inv_body_mass > 0.0f)
				{
					// Same effective-mass formula HandleContact uses (line 745-746): the inverse
					// effective mass at the contact point in the contact normal direction.
					Vec3 jacobian = Vec3(constraint->mContact->mPosition - body.GetCenterOfMassPosition()).Cross(plane_normal);
					float inv_eff_mass = body.GetInverseInertia().Multiply3x3(jacobian).Dot(jacobian) + inv_body_mass;
					if (inv_eff_mass > 0.0f)
					{
						const float m_eff = 1.0f / inv_eff_mass;
						clip_frac = m_eff / (m_eff + mMass);
					}
				}
				// inv_body_mass == 0 means the body is pinned to infinite mass — keep clip_frac = 1 (static-like).
			}
		}

		// The steep-slope horizontal-velocity cancel that lived here was removed (2026-06-11).
		// Upstream Jolt cancels all horizontal velocity into a too-steep face so characters can't
		// slide up steep slopes — but that converts legitimate MOMENTUM at a walkable→steep
		// transition (running up a curved ramp past the max slope angle) into a pure-vertical pop:
		// the horizontal dies in one frame, the accumulated climb vy carries, and the capsule
		// separates from the surface going straight up (logged 2026-06-11: z 9.0 → -0.25 in one
		// frame with vy 10.1 kept). The intended behavior is a plain tangent projection: momentum
		// slides UP the steep face, drained by gravity. "Can't climb steep slopes with input" is
		// enforced elsewhere now: the GDScript wall-projection trig cap shapes the input target off
		// steep faces, gravity is folded in every frame, and steep ground reads as not-grounded so
		// only (weaker) air accel applies — gravity along a >50° face out-pulls all of them.
		...
		// Calculate new velocity if we cancel the relative velocity in the normal direction (scaled by clip_frac per above).
		Vec3 new_velocity = velocity - clip_frac * relative_velocity.Dot(plane_normal) * plane_normal;

		// Inelastic momentum exchange bookkeeping for Dynamic bodies: after the mass-ratio clip both
		// the character and the body now share the same along-normal velocity. We update the
		// constraint's mLinearVelocity to reflect this so that on the next iteration
		// mProjectedVelocity = n·(mLinearVelocity − velocity) ≈ 0, and the constraint drops out of
		// the active set — instead of getting re-picked every iteration (with clip_frac ≈ 0.05 for
		// a light toad, mProjectedVelocity decays by only 5% per pass, so the solver would burn all
		// 15 iterations on a single contact producing zero displacement; that's the "brick wall +
		// can't walk away" stall this fixes). For Static / Kinematic / infinite-mass bodies
		// clip_frac == 1, so this matches the old behavior (mLinearVelocity stays its prior tangent
		// component plus the fully-cancelled normal component → constraint deactivates as before).
		if (constraint->mContact->mMotionTypeB == EMotionType::Dynamic)
		{
			Vec3 lin_tangent = constraint->mLinearVelocity - constraint->mLinearVelocity.Dot(plane_normal) * plane_normal;
			constraint->mLinearVelocity = lin_tangent + new_velocity.Dot(plane_normal) * plane_normal;
		}
		...
		// TEMP DEBUG: record the constraint we processed this iteration.
		if (dbg_on) {
			const Vec3 dn = constraint->mPlane.GetNormal();
			const Vec3 clv = constraint->mLinearVelocity;
			dbg_trace += ::vformat(" [it%d c%d n=(%.2f,%.2f,%.2f)%s toi=%.4f ot=%d pen=%.4f clv=(%.5f,%.5f,%.5f) mt=%d nz=%d -> v=(%.2f,%.2f,%.2f)]",
					(int)iteration, num_previous_contacts, (float)dn.GetX(), (float)dn.GetY(), (float)dn.GetZ(),
					constraint->mIsSteepSlope ? "S" : "", constraint->mTOI, other_constraint != nullptr ? 1 : 0, highest_penetration,
					(float)clv.GetX(), (float)clv.GetY(), (float)clv.GetZ(), (int)constraint->mContact->mMotionTypeB, clv.IsNearZero(1.0e-8f) ? 1 : 0,
					(float)velocity.GetX(), (float)velocity.GetY(), (float)velocity.GetZ());
		}
		...
		{
			DBG_SC_EXIT("velocity_dead");
			return;
		}

		// Only Static/Kinematic contacts contribute a meaningful "reversal reference": a moving platform's
		// surface moves rigidly with the body, so its contact-point velocity is a stable reference. Dynamic
		// bodies are excluded for two reasons:
		//   1) Static body push-out (n·|dist|/dt added by DetermineConstraints for penetrating contacts) is
		//      collision recovery, not world-applied velocity — using it as a reference clobbers last_velocity
		//      with a noise-level vector, and a subsequent clip's noise-level negative dot triggers a spurious
		//      reversal that zeroes the displacement (character pins against the next wall it slides into).
		//      That's why we read mContact->mLinearVelocity rather than the constraint's (the underlying contact's
		//      mLinearVelocity is body velocity only — no push-out — so Static contacts have ~0 here and fall
		//      through to the prior reference).
		//   2) Dynamic body mLinearVelocity is the velocity *at the contact point* = linear + ω×r (see line 221).
		//      A spinning / rolling toad's contact point can be moving in any direction with significant magnitude
		//      even when the body itself is barely translating, so the post-clip player velocity easily ends up
		//      "opposing" that arbitrary vector — triggering a real-magnitude reversal that wasn't a true
		//      direction-flip of the character. The result is the player getting frozen against a pushable dynamic
		//      body (brick-wall feel) instead of sliding past it as the body yields to the push impulse.
		// The reversal early-out that used to live here (else if velocity.Dot(last_velocity) < 0 → return)
		// was removed. Its job was terminating ping-pong oscillation, but with the listener's
		// once-per-contact-per-frame re-seat gate every solve pass is strictly non-expansive, so
		// oscillation decays geometrically and the loop exits via velocity_dead / goal / time on its
		// own. What the early-out still did in practice was freeze legitimate motion: after a
		// multi-plane corner clip (floor + wall + steep face) the small residual velocity can point
		// slightly "backward" vs the frame input (measured cosine ≈ -0.12 at noise magnitude), and the
		// early-out zeroed the entire frame's displacement — the wall-slide stalls of 2026-06-10. The
		// residual motion it suppressed is constraint-legal by construction (every processed plane is
		// respected via the previous-contact crease handling above). last_velocity is kept updated for
		// the [SC] debug trace only.
		if (constraint->mContact->mMotionTypeB != EMotionType::Dynamic && !constraint->mContact->mLinearVelocity.IsNearZero(1.0e-8f))
			last_velocity = constraint->mContact->mLinearVelocity;
	}
	DBG_SC_EXIT("max_iterations");
	#undef DBG_SC_EXIT
}
...
void CharacterVirtual::UpdateSupportingContact(bool inSkipContactVelocityCheck, TempAllocator &inAllocator)
{
	...
	for (const Contact &c : mActiveContacts)
		if (c.mHadCollision && !c.mWasDiscarded)
		{
			...
			// Check if this is a sliding or supported contact. Dynamic too-steep contacts (e.g., the side
			// of a pushable prop you're walking into) are not slopes you can slide off — they're props.
			// Excluding them here prevents the OnSteepGround classification from triggering on a sphere
			// toad you're trying to push, which would otherwise have CancelVelocityTowardsSteepSlopes
			// erase your horizontal velocity (the "stopped and bumped around" failure mode). A walkable-
			// surface contact on a Dynamic body (sn.y >= cos(maxSlope), e.g., standing on top of a block)
			// still counts as num_supported and gives OnGround as expected.
			bool is_supported = mCosMaxSlopeAngle > cNoMaxSlopeAngle || cos_angle >= mCosMaxSlopeAngle;
			if (is_supported)
				num_supported++;
			else if (c.mMotionTypeB != EMotionType::Dynamic)
				num_sliding++;
	...
	)
{
	...
	for (uint iteration = 0; iteration < mMaxCollisionIterations && time_remaining >= mMinTimeRemaining; iteration++)
	{
		...
		Vec3 dbg_disp_solve = displacement; // TEMP DEBUG
		float dbg_sweep_frac = 1.0f;        // TEMP DEBUG
		...
		{
			// The verification sweep exists to catch NEW obstructions — bodies the swept volume would pass
			// through that weren't in the contact set when the constraint solver ran. It should NOT re-clip
			// against bodies the constraint solver already handled: those have already had a tangent /
			// mass-ratio-clipped displacement computed for them, and a re-hit (especially at fraction ≈ 0
			// against a curved Dynamic body the capsule is flush against) just zeroes the motion redundantly
			// and produces "stop-and-start" against pushable props. Skip the clip when the first hit's body
			// is already in `contacts`. This is motion-type-agnostic: a heavy Dynamic wall is in the contact
			// set → skip → constraint's tangent clip stands → planar slide is clean; a light Dynamic toad is
			// in the contact set → skip → constraint's mass-ratio clip stands → player phases through smoothly;
			// a Static wall is in the contact set → skip → planar slide unchanged. A genuinely new obstruction
			// (a body NOT in the contact set, e.g. one that entered the swept volume) still clips correctly.
			// (Clipping removed. The verification sweep was supposed to be a safety net against bodies the
			// constraint solver didn't know about, but in practice it ran against bodies that WERE in the contact
			// set and re-clipped them — which is at best redundant (planar walls: constraint already produced
			// exactly-tangent displacement, sweep returns mFraction = 1.0 anyway) and at worst destructive (curved
			// Dynamic bodies flush against the capsule: sweep returns mFraction ~ 0 and zeroes the displacement
			// every frame, producing stop-and-start against pushable props). Trust the constraint solver as the
			// sole motion authority. Catching genuinely-new obstructions along the swept path is the job of
			// mPredictiveContactDistance, which must be sized to cover the per-frame displacement.)
			dbg_sweep_frac = cast_contact.mFraction; // TEMP DEBUG (kept for log visibility, no clip applied)
		}

		// TEMP DEBUG: did the verification sweep kill a non-trivial solved move,
		// or did SolveConstraints itself produce ~nothing from a real velocity?
		{
			const float dbg_solve_sq = dbg_disp_solve.LengthSq();
			const float dbg_final_sq = displacement.LengthSq();
			const float dbg_in_h_sq = inVelocity.GetX() * inVelocity.GetX() + inVelocity.GetZ() * inVelocity.GetZ();
			if (dbg_in_h_sq > 4.0f && (dbg_solve_sq < 1.0e-6f || dbg_final_sq < 0.01f * dbg_solve_sq)) {
				::print_line(::vformat("[MS] it=%d in_vel=(%.3f,%.3f,%.3f) solve_disp=(%.5f,%.5f,%.5f) |%.5f| sweep_frac=%.5f final_disp=(%.5f,%.5f,%.5f) |%.5f| ncon=%d",
						(int)iteration,
						(float)inVelocity.GetX(), (float)inVelocity.GetY(), (float)inVelocity.GetZ(),
						(float)dbg_disp_solve.GetX(), (float)dbg_disp_solve.GetY(), (float)dbg_disp_solve.GetZ(), (float)dbg_disp_solve.Length(),
						dbg_sweep_frac,
						(float)displacement.GetX(), (float)displacement.GetY(), (float)displacement.GetZ(), (float)displacement.Length(),
...
void CharacterVirtual::ExtendedUpdate(float inDeltaTime, Vec3Arg inGravity, const ExtendedUpdateSettings &inSettings, const BroadPhaseLayerFilter &inBroadPhaseLayerFilter, const ObjectLayerFilter &inObjectLayerFilter, const BodyFilter &inBodyFilter, const ShapeFilter &inShapeFilter, TempAllocator &inAllocator)
{
	...
	// Capture desired velocity for WalkStairs (below). Previously this also passed through
	// CancelVelocityTowardsSteepSlopes which zeroed any horizontal velocity pointing into a too-steep
	// contact's surface, but that was removed: the constraint solver already handles too-steep slopes
	// reactively in DetermineConstraints + SolveConstraints (the secondary vertical-plane constraint
	// at line ~681 blocks horizontal-into-slope motion during MoveShape). The pre-emptive cancel was
	// both redundant and behaved differently for Dynamic vs Static contacts — for a sphere toad the
	// player was leaning against, it erased the player's whole horizontal velocity rather than
	// reactively clipping it, producing the "stopped and bumped around" failure mode. Removing it
	// makes the slope behavior uniform across Static and Dynamic and leaves gravity to slide the
	// character down too-steep slopes.
